...
def test_create_appointment_success(service, db_session):
    ...
    ), f"Could not retrieve appointment with ID {db_appt.appointment_id}"
...
def test_update_status_valid_transitions(service, db_session):
    ...
    assert (
        db_appt is not None
    ), f"Appointment {appt.appointment_id} not found after update"
...
def test_update_appointment_conflict(service, db_session):
    ...
    second_appt = db_session.exec(
        select(Appointment).where(Appointment.id != first_appt.id)
    ).first()
