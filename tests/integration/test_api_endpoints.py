...
def test_crud_and_counts(client, valid_appointment_payload):
    ...
    body = lst.json()
    print(
        f"🔍 Found {body.get('total', 0)} appointments, first page has {len(body.get('appointments', []))} appointments"
    )
    assert (
        body["total"] >= 1
    ), f"Expected at least 1 appointment, got {body.get('total')}"
    assert (
        len(body["appointments"]) >= 1
    ), f"Expected at least 1 appointment in response, got {len(body.get('appointments', []))}"
    ...
    ), f"Expected appointment_id {appointment_id}, got {response_data.get('appointment_id')}"
