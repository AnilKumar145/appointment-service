...
def test_wrong_content_type_falls_back_or_errors_cleanly(
...
@pytest.mark.security
def test_extra_unexpected_fields_are_ignored_or_rejected(
    client, valid_appointment_payload
):
...
def test_invalid_enum_values_rejected_by_status_update(client, create_appointment):
    ...
    resp = client.put(
        f"/api/appointments/{appt_id}/status", json={"status": "NOT_A_STATUS"}
    )
...
@pytest.mark.security
def test_oversized_payload_is_rejected_or_safely_handled(
    client, valid_appointment_payload
):
    huge = "x" * 20000
