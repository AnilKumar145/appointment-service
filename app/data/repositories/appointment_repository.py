...
class AppointmentRepository:
    ...
    def get_by_status(
        self, status: AppointmentStatus, skip: int = 0, limit: int = 100
    ) -> List[Appointment]:
    ...
    def count_by_doctor(
        ...
        if status:
            statement = select(Appointment).where(
                Appointment.doctor_id == doctor_id, Appointment.status == status
            )
        else:
            statement = select(Appointment).where(Appointment.doctor_id == doctor_id)
        return len(list(self.session.exec(statement).all()))
    ...
    def count_by_patient(
        ...
        if status:
            statement = select(Appointment).where(
                Appointment.patient_id == patient_id, Appointment.status == status
            )
        else:
            statement = select(Appointment).where(Appointment.patient_id == patient_id)
        return len(list(self.session.exec(statement).all()))
