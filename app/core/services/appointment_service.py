...
class AppointmentService:
    ...
    def get_all_appointments(
        ...
        try:
            ...
            appointment_responses = [
                self._to_response_model(appt) for appt in appointments
            ]
    ...
    def get_appointment_by_id(self, appointment_id: str) -> AppointmentResponse:
        ...
        self.logger.info(
            f"🔍 Looking up appointment with ID: {appointment_id} (type: {type(appointment_id)})"
        )
        appointment = self.repository.get_by_id(appointment_id)
        if not appointment:
            self.logger.error(
                f"❌ Appointment with ID {appointment_id} not found in database"
            )
    ...
    def get_appointments_by_facility_id(
    ...
    def get_available_time_slots(
        self, doctor_id: str, appointment_date: date
    ) -> List[dict]:
        ...
        try:
            ...
            booked_appointments = self.repository.get_available_slots(
            ...
            while current_hour < end_hour:
                ...
                for appointment in booked_appointments:
                    ...
                    if (
                        slot_start_time < appointment.appointment_end_time
                        and slot_end_time > appointment.appointment_start_time
                    ):
                        is_available = False
                        print(
        ...
        try:
            ...
            if not validate_business_hours(
                ...
                raise BusinessRuleViolationError(
                    "Appointment must be scheduled within business hours (9 AM - 6 PM)"
                )
            ...
            raise ValidationError(
                "Failed to generate a unique appointment_id after multiple attempts"
            )
    ...
    def update_appointment_status(
        self, appointment_id: str, status_update: AppointmentStatusUpdate
    ) -> AppointmentResponse:
        ...
        valid_transitions = {
            ...
            AppointmentStatus.CANCELLED: [
                AppointmentStatus.SCHEDULED
            ],  # Can reschedule cancelled
        }

        if (
            appointment.status in valid_transitions
            and status_update.status not in valid_transitions[appointment.status]
        ):
            raise ValidationError(
                f"Cannot change status from {appointment.status} to {status_update.status}"
            )
    ...
    def update_appointment(
        self, appointment_id: str, appointment_data: AppointmentUpdate
    ) -> AppointmentResponse:
        ...
        if (
            appointment_data.appointment_start_time
            or appointment_data.appointment_end_time
            or appointment_data.appointment_date
        ):
    ...
    def get_appointment_count_for_doctor(
        self, doctor_id: str, status: Optional[AppointmentStatus] = None
    ) -> int:
    ...
    def get_appointment_count_for_patient(
        self, patient_id: str, status: Optional[AppointmentStatus] = None
    ) -> int:
    ...
    def _format_id(self, id_str: str, prefix: str) -> str:
        ...
        if re.match(rf"^{prefix}-\d{{4}}-\d{{4}}$", id_str):
            return id_str

        # If missing hyphens, try to insert them
        match = re.match(rf"^{prefix}(\d{{4}})(\d{{4}})$", id_str.replace("-", ""))
        if match:
